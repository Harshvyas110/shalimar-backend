...
class FMPService {
  constructor() {
    console.log('✅ FMPService initialized (API Key: ' + FMP_API_KEY.substring(0, 10) + '...)');
  }

  /**
   * Get historical daily candles from FMP
   */
  async getDailyCandles(symbol) {
    try {
      ...
      console.error(`[FMP] ❌ Daily candles error for ${symbol}:`, error.message);
      throw error;
    }
  }

  /**
   * Get intraday candles from FMP (1-min, 5-min, 15-min, etc.)
   */
  async getIntradayCandles(symbol, interval = '15min') {
    try {
      console.log(`[FMP] Fetching ${interval} intraday candles for ${symbol}...`);

      const response = await axios.get(
        `${FMP_URL}/historical-chart/${interval}/${symbol.toUpperCase()}`,
        {
          params: {
            apikey: FMP_API_KEY,
          },
      ...
      console.log(
        `[FMP] ✅ Got ${candles.length} ${interval} candles for ${symbol}`
      );
      return candles;
    } catch (error) {
      console.error(
        `[FMP] ❌ Intraday candles error for ${symbol}:`,
        error.message
      );
      throw error;
    }
